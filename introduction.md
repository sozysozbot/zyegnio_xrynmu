...
隋拼では、主母音にアキュートアクセント(áéə́íóúý)を付けて上声を表記し、主母音にサーカムフレックス(âêə̂îôûŷ)を付けて去声を表記します。
...
* 綻,trên「介音にr性がある」のでtr類。頭子音はtrで韻母がrên。
...
### l類
l類はlのみからなります。特徴として、介音にr性が**基本的には**来ない、というのがあります。
...
・「冷」： l（l類） + ráŋ
・などなど
これらは、現在はそれぞれ `dʼî`, `tʼráŋ`, `lráŋ` と書くことになっています（`l`類は「**基本的には**」に過ぎないので`ʼ`を挿入しない）。

追記：
Magnezone462「というか登場頻度が極端に低いので割となんでもいいという。誤解がなければなんでも`t-r`でも`t'r`でも。`'`はむかし形態表記のためにどかしたけれども不採用になったという経緯が。」とのことだそうな。

追記2: [長くなったので別記事に分離](https://github.com/sozysozbot/zyegnio_xrynmu/blob/master/tryikiə̂.md)
